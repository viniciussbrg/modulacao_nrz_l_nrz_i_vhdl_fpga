...
- **`src/top_nrz_l_basys3.vhd`** — Estendido com suporte a clock, gerador de sincronismo VGA e lógica de serialização temporizada. Exibe cada bit como cor de tela inteira por 2 segundos, do SW15 ao SW0
- **`constraints/basys3_nrz_l.xdc`** — Estendido com o mapeamento do clock da placa e dos pinos do conector VGA.
Os demais arquivos são idênticos aos do projeto principal.
 
## Arquivos de código
 
- **`src/nrz_l.vhd`** — Encoder NRZ-L (idêntico ao projeto principal)
- **`src/top_nrz_l_basys3.vhd`** — Top-level com saída para LEDs, gerador VGA e serialização temporizada por bit
- **`sim/tb_nrz_l.vhd`** — Testbench do encoder (idêntico ao projeto principal)
- **`constraints/basys3_nrz_l.xdc`** — Mapeamento de pinos da Basys 3, incluindo clock e conector VGA
...
> A simulação cobre apenas o encoder NRZ-L (`nrz_l.vhd`). O gerador VGA e a serialização temporizada do top-level não são simulados pelo testbench.
...
6. Clique em **Program Device** → selecione o arquivo `top_nrz_l_basys3.bit` → clique em **Program**

Após a gravação, o monitor exibirá imediatamente a cor correspondente ao bit SW15.
...
| Bit (SW) | Nível NRZ-L | Cor da tela |
|:-:|:-:|:-:|
| 1 | Alto | Azul inteiro |
| 0 | Baixo | Vermelho inteiro |

A sequência percorre os 16 bits de SW15 a SW0, levando 32 segundos no total, e então reinicia do SW15. Na NRZ-L não há lógica de transição: a cor exibida é o reflexo direto do valor do switch correspondente.

**Nos LEDs da placa:**

Os LEDs LD15 a LD0 espelham diretamente os switches correspondentes (`led <= sw`), funcionando independentemente da saída VGA.
...
Exemplo correto:
```tcl
cd {C:/Users/aluno/projetos/nrz_l/extras/VGA/vivado_project}
```

Exemplo incorreto (não use):
```tcl
cd {C:\Users\aluno\projetos\nrz_l\extras\VGA\vivado_project}
```
...
```
======================================================================
 Projeto NRZ_L_VGA criado com sucesso!
======================================================================
```
...
A constante `CYCLES_PER_BIT` no `top_nrz_l_basys3.vhd` controla a duração de cada bit na tela. Para alterar, edite o valor no arquivo antes de recompilar:
